...
export default function HomePage() {
  ...
  const [chartMode, setChartMode] = useState('single_segmented'); // 'single_segmented' or 'multi_metric'
  const [chartMetric, setChartMetric] = useState('totalValue'); // The metric for single_segmented mode
  const [segmentationProperty, setSegmentationProperty] = useState('companyCountry'); // The breakdown for single_segmented mode
  const [multiMetrics, setMultiMetrics] = useState({ // The metrics for multi_metric mode
    totalValue: true,
    totalDeals: true,
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setIsLoading(true);
      // For now, we only pass the main filters. We will add the chart config later.
      const config = { outcome, timePeriod, companyCountry, numberOfEmployees };
      try {
    ...
    fetchData();
  }, [outcome, timePeriod, companyCountry, numberOfEmployees]); // Note: chart config state is not here yet
    ...
    <main className="flex h-screen bg-gray-900 text-gray-300 font-sans">
      <div className="w-1/3 max-w-sm p-6 bg-gray-800 shadow-lg overflow-y-auto">
        ...
        <div className="space-y-6">
            {/* ... (Report Type, Outcome, and Filters sections remain the same) ... */}
          ...
          <div>
            ...
            <fieldset className="space-y-2">
              <legend className="text-sm font-medium text-gray-400">Chart Mode</legend>
              <div className="flex items-center space-x-4">
                <label className="flex items-center"><input type="radio" name="chartMode" value="single_segmented" checked={chartMode === 'single_segmented'} onChange={(e) => setChartMode(e.target.value)} className="h-4 w-4 text-indigo-600 border-gray-300"/><span className="ml-2">Breakdown</span></label>
                <label className="flex items-center"><input type="radio" name="chartMode" value="multi_metric" checked={chartMode === 'multi_metric'} onChange={(e) => setChartMode(e.target.value)} className="h-4 w-4 text-indigo-600 border-gray-300"/><span className="ml-2">Multiple Metrics</span></label>
              </div>
            </fieldset>
            
            {/* Conditional UI for Single Metric, Segmented Mode */}
            {chartMode === 'single_segmented' && (
              <div className="mt-4 space-y-4">
                <div><label htmlFor="chartMetric" className="block text-sm font-medium text-gray-400">Metric</label><select id="chartMetric" name="chartMetric" value={chartMetric} onChange={(e) => setChartMetric(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base bg-gray-700 border-gray-600 text-white focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"><option value="totalValue">Total Value</option><option value="totalDeals">Total Deals</option><option value="influencedValue">Influenced Value</option></select></div>
                <div><label htmlFor="segmentationProperty" className="block text-sm font-medium text-gray-400">Breakdown by</label><select id="segmentationProperty" name="segmentationProperty" value={segmentationProperty} onChange={(e) => setSegmentationProperty(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base bg-gray-700 border-gray-600 text-white focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"><option value="companyCountry">Company Country</option><option value="numberOfEmployees">Number of Employees</option></select></div>
              </div>
            )}
            
            {/* Conditional UI for Multiple Metrics Mode */}
            {chartMode === 'multi_metric' && (
              <div className="mt-4 space-y-2">
                <label className="flex items-center"><input type="checkbox" name="totalValue" checked={multiMetrics.totalValue} onChange={handleMultiMetricChange} className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" /><span className="ml-2">Total Value</span></label>
                <label className="flex items-center"><input type="checkbox" name="totalDeals" checked={multiMetrics.totalDeals} onChange={handleMultiMetricChange} className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" /><span className="ml-2">Total Deals</span></label>
                <label className="flex items-center"><input type="checkbox" name="influencedValue" checked={multiMetrics.influencedValue} onChange={handleMultiMetricChange} className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" /><span className="ml-2">Influenced Value</span></label>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="mt-8"><Chart data={chartData} /></div>
