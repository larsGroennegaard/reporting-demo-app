...
interface SelectedOutcomeMetrics { [stageName: string]: ('deals' | 'value')[]; }
interface SelectedInfluencedMetrics { [stageName: string]: ('deals' | 'value')[]; }
...
export default function HomePage() {
  ...
  const [outcomeMetrics, setOutcomeMetrics] = useState<SelectedOutcomeMetrics>({ 'NewBiz': ['deals', 'value'], 'SQL': ['deals'] });
  ...
  const [influencedMetrics, setInfluencedMetrics] = useState<SelectedInfluencedMetrics>({});
  ...
  // --- Memoized values for deriving available metrics and dependencies ---
  ...
  // Dependency strings for main useEffect
  const outcomeMetricsDep = JSON.stringify(outcomeMetrics);
  const kpiConfigDep = JSON.stringify(kpiCardConfig);
  const countriesDep = selectedCountries.join(',');
  const employeesDep = selectedEmployeeSizes.join(',');
  const multiChartMetricsDep = multiChartMetrics.join(',');
  const engagementMetricsDep = engagementMetrics.join(',');
  const influencedMetricsDep = JSON.stringify(influencedMetrics);
  const eventNamesDep = selectedEventNames.join(',');
  const signalsDep = selectedSignals.join(',');
  ...
  useEffect(() => {
    // Set initial state when switching archetypes
    setIsLoading(true);
    setKpiData(null);
    setChartData([]);
    setKpiCardConfig([]);
    ...
    if (reportArchetype === 'outcome_analysis') {
      // Set defaults for Outcome Analysis
      if(availableMetrics.length > 0) {
        setSingleChartMetric(availableMetrics.includes('NewBiz_value') ? 'NewBiz_value' : availableMetrics[0]);
       ...
       // Set defaults for Engagement Analysis
       if(availableMetrics.length > 0) {
        setSingleChartMetric(availableMetrics.includes('companies') ? 'companies' : availableMetrics[0]);
        setMultiChartMetrics(['companies', 'contacts'].filter(m => availableMetrics.includes(m)));
         setKpiCardConfig([
          { id: 1, metric: 'companies' }, { id: 2, metric: 'contacts' }, { id: 3, metric: 'events' },
        ].filter(c => availableMetrics.includes(c.metric)));
       }
    }
  }, [reportArchetype, availableMetrics.join(',')]); // Rerun when available metrics change for the archetype

  useEffect(() => {
    // This effect handles the API call
    const fetchData = async () => {
      ...
      setIsLoading(true);
      try {
        console.log("Fetching with config:", currentConfig);
    ...
    // Debounce fetching to prevent rapid firing while user is clicking checkboxes
  ...
  }, [reportArchetype, reportFocus, timePeriod, kpiConfigDep, singleChartMetric, multiChartMetricsDep, segmentationProperty, outcomeMetricsDep, countriesDep, employeesDep, engagementMetricsDep, influencedMetricsDep, eventNamesDep, signalsDep, urlFilter]);

  // --- Handlers ---
  const handleOutcomeMetricChange = (stageName: string, metricType: 'deals' | 'value') => { setOutcomeMetrics(prev => { const newConfig = JSON.parse(JSON.stringify(prev)); const stageMetrics = newConfig[stageName] || []; if (stageMetrics.includes(metricType)) { newConfig[stageName] = stageMetrics.filter((m: string) => m !== metricType); if (newConfig[stageName].length === 0) { delete newConfig[stageName]; } } else { newConfig[stageName] = [...stageMetrics, metricType]; } return newConfig; }); };
  ...
  const handleInfluencedMetricChange = (stageName: string, metricType: 'deals' | 'value') => { setInfluencedMetrics(prev => { const newConfig = JSON.parse(JSON.stringify(prev)); const stageMetrics = newConfig[stageName] || []; if (stageMetrics.includes(metricType)) { newConfig[stageName] = stageMetrics.filter((m: string) => m !== metricType); if (newConfig[stageName].length === 0) { delete newConfig[stageName]; } } else { newConfig[stageName] = [...stageMetrics, metricType]; } return newConfig; }); };
  ...
  const renderOutcomeAnalysisConfig = () => (
  ...
  const renderEngagementAnalysisConfig = () => (
      <div className="contents">
          <div>
            <button onClick={() => setIsMetricsOpen(!isMetricsOpen)} className="w-full flex justify-between items-center text-left text-lg font-semibold text-gray-100 hover:text-white"><span>Metrics</span><span className="text-xs">{isMetricsOpen ? '▼' : '►'}</span></button>
            {isMetricsOpen && (
              <div className="mt-2 space-y-4 border-l-2 border-gray-700 pl-4">
                <div>
                  ...
                  <div className="pl-2 mt-1 space-y-1">
                    <label className="flex items-center"><input type="checkbox" checked={engagementMetrics.includes('companies')} onChange={() => handleEngagementMetricChange('companies')} className="h-4 w-4 rounded border-gray-300 text-indigo-600" /><span className="ml-2">Companies</span></label>
                    <label className="flex items-center"><input type="checkbox" checked={engagementMetrics.includes('contacts')} onChange={() => handleEngagementMetricChange('contacts')} className="h-4 w-4 rounded border-gray-300 text-indigo-600" /><span className="ml-2">Contacts</span></label>
                    <label className="flex items-center"><input type="checkbox" checked={engagementMetrics.includes('events')} onChange={() => handleEngagementMetricChange('events')} className="h-4 w-4 rounded border-gray-300 text-indigo-600" /><span className="ml-2">Events</span></label>
                  </div>
                </div>
                <div>
                  <p className="font-medium text-white">Influenced Stage Metrics</p>
                  {stageOptions.map(stage => (
                    <div key={stage} className="pl-2 mt-1">
                      <p className="font-medium text-gray-300">{stage}</p>
                      <div className="pl-2 mt-1 space-y-1">
                        <label className="flex items-center"><input type="checkbox" checked={influencedMetrics[stage]?.includes('deals')} onChange={() => handleInfluencedMetricChange(stage, 'deals')} className="h-4 w-4 rounded border-gray-300 text-indigo-600" /><span className="ml-2"># Touched Deals</span></label>
                        <label className="flex items-center"><input type="checkbox" checked={influencedMetrics[stage]?.includes('value')} onChange={() => handleInfluencedMetricChange(stage, 'value')} className="h-4 w-4 rounded border-gray-300 text-indigo-600" /><span className="ml-2">Touched Value</span></label>
                      </div>
                    </div>
                  ))}
                </div>
  ...
  const renderSharedSettings = () => (
    <>
      <div>
        <h3 className="text-lg font-semibold mb-2 text-gray-100">Report Focus</h3>
        <fieldset className="flex gap-4">
            <label className="flex items-center"><input type="radio" name="reportFocus" value="time_series" checked={reportFocus === 'time_series'} onChange={(e) => setReportFocus(e.target.value)} className="h-4 w-4 text-indigo-600 border-gray-300"/><span className="ml-2">Time Series</span></label>
            <label className="flex items-center"><input type="radio" name="reportFocus" value="segmentation" checked={reportFocus === 'segmentation'} onChange={(e) => setReportFocus(e.target.value)} className="h-4 w-4 text-indigo-600 border-gray-300"/><span className="ml-2">Segmentation</span></label>
        </fieldset>
      </div>
      <div>
        ...
        {isChartTableSettingsOpen && <div className="mt-2 space-y-4 border-l-2 border-gray-700 pl-4 pt-2">
            {reportFocus === 'time_series' && (
              <div>
                <h3 className="text-md font-semibold mb-2 text-gray-200">Time Series Chart</h3>
                <fieldset className="space-y-2"><legend className="text-sm font-medium text-gray-400">Chart Mode</legend><div className="flex items-center space-x-4"><label className="flex items-center"><input type="radio" name="chartMode" value="single_segmented" checked={chartMode === 'single_segmented'} onChange={(e) => setChartMode(e.target.value)} className="h-4 w-4 text-indigo-600 border-gray-300"/><span className="ml-2">Breakdown</span></label><label className="flex items-center"><input type="radio" name="chartMode" value="multi_metric" checked={chartMode === 'multi_metric'} onChange={(e) => setChartMode(e.target.value)} className="h-4 w-4 text-indigo-600 border-gray-300"/><span className="ml-2">Multiple Metrics</span></label></div></fieldset>
      ...
      <div>
        ...
        <select id="timePeriod" value={timePeriod} onChange={(e) => setTimePeriod(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base bg-gray-700 border-gray-600 text-white focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md"><option value="this_year">This Year</option><option value="last_quarter">Last Quarter</option><option value="last_month">Last Month</option></select>
      </div>
    </>
  );
      ...
      <div className="w-1/3 max-w-sm p-6 bg-gray-800 shadow-lg overflow-y-auto">
        ...
        <div className="space-y-6">
          <div>
            ...
            <fieldset className="flex gap-4">
              <label className="flex items-center"><input type="radio" name="reportArchetype" value="outcome_analysis" checked={reportArchetype === 'outcome_analysis'} onChange={(e) => setReportArchetype(e.target.value)} className="h-4 w-4 text-indigo-600 border-gray-300" /><span className="ml-2">Outcome Analysis</span></label>
              <label className="flex items-center"><input type="radio" name="reportArchetype" value="engagement_analysis" checked={reportArchetype === 'engagement_analysis'} onChange={(e) => setReportArchetype(e.target.value)} className="h-4 w-4 text-indigo-600 border-gray-300" /><span className="ml-2">Engagement Analysis</span></label>
            </fieldset>
          </div>
          
          {reportArchetype === 'outcome_analysis' ? renderOutcomeAnalysisConfig() : renderEngagementAnalysisConfig()}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {kpiCardConfig.map(card => (
                    <div key={card.id} className="bg-gray-800/50 p-2 rounded-lg border border-gray-700">
                        ...
                        <select value={card.metric} onChange={(e) => updateKpiCardMetric(card.id, e.target.value)} className="mb-2 block w-full text-xs bg-gray-700 border-gray-600 text-white focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 rounded-md">
